...
type OverviewRow = {
  bookingId: string; // ✅ Booking ID = quoteReference
  vehicleModel: string;
  pickupLocation: string;
  dropoffLocation: string;
  date: string;
...
const extractDashboardStats = (json: any): DashboardStats => {
  ...
  if (maybe?.stats && Array.isArray(maybe.stats)) {
    const getCount = (name: string) => {
      const found = maybe.stats.find(
        (s: any) =>
          String(s?.category || "").toLowerCase() === name.toLowerCase()
      );
      const n = found?.totalCount;
      return typeof n === "number" ? n : 0;
    };
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        if (!dashRes.ok) {
          throw new Error(
            dashJson?.message || dashText || "Failed to load dashboard"
          );
        }
        ...
        const mapped: OverviewRow[] = (Array.isArray(list) ? list : []).map(
          (r: any) => ({
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        />
        <StatCard
          title="Total Successful (Shipped)"
      ...
      <div className="bg-white rounded-lg shadow-sm">
        ...
        {/* ✅ 7 columns now */}
        <div className="grid grid-cols-7 gap-4 px-6 py-3 text-sm text-muted-foreground border-b">
          <div>Booking ID</div>
          <div>Vehicle Model</div>
          <div>Location</div>
          <div>Destination</div>
        ...
        {!rows.length ? (
          ...
          rows.map((r, idx) => (
            ...
            >
              <div className="truncate">{r.bookingId}</div>
              <div className="truncate">{r.vehicleModel}</div>
              <div className="truncate">{r.pickupLocation}</div>
              <div className="truncate">{r.dropoffLocation}</div>
              <div className="truncate">{r.date}</div>
